...
public class TextPrompts
{
    ...
    [McpServerPrompt(IconSource = "https://raw.githubusercontent.com/microsoft/fluentui-emoji/main/assets/Memo/Flat/memo_flat.svg"), Description("Reverses a single word. Output is the reversed word only — no explanation, no punctuation.")]
    public string ReverseWord([Description("Single word to reverse")] string word)
    ...
    /// <returns>A prompt string.</returns>
    [McpServerPrompt(IconSource = "https://raw.githubusercontent.com/microsoft/fluentui-emoji/main/assets/Memo/Flat/memo_flat.svg"), Description("Summarises text in one sentence (≤25 words).")]
    public string OneSentenceSummary([Description("Text to summarise")] string text)
    ...
    /// <returns>A prompt string.</returns>
    [McpServerPrompt(IconSource = "https://raw.githubusercontent.com/microsoft/fluentui-emoji/main/assets/Memo/Flat/memo_flat.svg"), Description("Generates a Markdown document with a summary paragraph, bullet list of key benefits, and a reference list for the given topic.")]
    public string SummaryBenefitsAndReferences([Description("Topic to research and document")] string topic)
